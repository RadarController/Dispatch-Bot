...
async function getRolePanelConfig(guildId) {
  return (await store.readGuildState(guildId)).rolePanelConfig || store.getDefaultGuildState().rolePanelConfig;
}

async function updateRolePanelConfig(guildId, updater) {
  return store.updateGuildState(guildId, (guildState) => {
    const currentConfig = guildState.rolePanelConfig || store.getDefaultGuildState().rolePanelConfig;
